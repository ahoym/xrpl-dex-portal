 ...
 * and devnet, and a community-operated cluster for mainnet. The application
 * trusts these endpoints implicitly — all ledger queries and transaction
 * submissions flow through them. If these URLs were changed (e.g., via a
 * compromised dependency or malicious PR), an attacker could intercept
 * transactions and wallet seeds. Review changes to this file carefully.
 */
...
export function resolveNetwork(network?: string): NetworkId {
  if (network && !(network in NETWORKS)) {
    console.debug(`Unknown network "${network}", falling back to ${DEFAULT_NETWORK}`);
  }
  if (network && network in NETWORKS) {
