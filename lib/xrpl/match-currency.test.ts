...
describe("matchesCurrency", () => {
  it("matches XRP regardless of issuer", () => {
    ...
    expect(matchesCurrency({ currency: "XRP", issuer: "rSomeAddress" }, "XRP", undefined)).toBe(true);
  });
  ...
  it("handles hex-encoded currency codes", () => {
    ...
    expect(
      matchesCurrency(
        { currency: hexRLUSD, issuer: "rIssuer1" },
        "RLUSD",
        "rIssuer1",
      ),
    ).toBe(true);
  ...
  it("matches raw hex currency code without decoding", () => {
    ...
    expect(
      matchesCurrency(
        { currency: hexCode, issuer: "rIssuer1" },
        hexCode,
        "rIssuer1",
      ),
    ).toBe(true);
