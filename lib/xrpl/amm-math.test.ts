...
describe("buildAmmPoolParams", () => {
  ...
  it("returns null for frozen base asset", () => {
    expect(buildAmmPoolParams(makePool({ asset1Frozen: true }))).toBeNull();
  });
  ...
  it("returns null for zero base reserves", () => {
    expect(buildAmmPoolParams(makePool({ asset1Value: "0" }))).toBeNull();
  });

  it("returns null for zero quote reserves", () => {
    expect(buildAmmPoolParams(makePool({ asset2Value: "0" }))).toBeNull();
  });
