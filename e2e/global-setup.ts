...
setup("generate wallet and trust RLUSD", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /^r[a-zA-Z0-9]{24,}/ })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(page.getByRole("button", { name: /RLUSD \(trusted\)/ })).toBeVisible({ timeout: 20_000 });
